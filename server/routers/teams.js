...
router.post('/', async (req, res, next) => {
  ...
  try
  {
    //Se ia numele de utilizator
    const username = req.username
    //Se cauta utilizatorul in bd
    const user = await User.findOne({ where: { username: username } });
    //Daca un utilizator este profesor, aditia se poate intampla
    if (user && user.is_professor == 1) {
      ...
      if (existantTeam !== null) {
        return res.status(400).send({ "message": "Numele echipei sau al proiectului este deja ales" })
      }
      else {
        ...
        team.setJury(jury);
        await team.save();
        return res.status(200).send({ "message": "Echipa " + team.name + " a fost creata." })
      }
    }
    else {
...
router.delete('/:id', async (req, res, next) => {
  ...
  try
  {
    ...
    if (user && user.is_professor == 1) {
      ...
      return res.status(200).send({ "message": "Echipa a fost stearsa." })
    }
    else {
      return res.status(401).send(apiError.Unauthorized);
    }
...
router.post('/:id/members', async (req, res, next) => {
  ...
  try
  {
    ...
    return res.status(200).json({ message: 'Membrii echipei au fost schimbati cu succes.' });
  }
  catch(err)
  {
    next(err);
...
router.post('/:id/deliverables', async (req, res, next) => {
  ...
  try
  {
    ...
    return res.status(200).send({ "message": del.title + " a fost creeat." })
  }
  catch(err)
  {
    next(err);
...
router.put('/:id/deliverables/:deliverableId', async (req, res, next) => {
  ...
  try
  {
    ...
    return res.status(200).send({ "message": "Schimbarile au fost facute cu succes." })
  }
  catch(err)
  {
    next(err);
...
router.delete('/:id/deliverables', async (req, res, next) => {
  ...
  try
  {
    ...
    return res.status(200).send({ "message": "Livrabilele echipei au fost schimbate cu succes." })
  }
  catch(err)
  {
    next(err);
...
router.post('/:id/juries', async (req, res, next) => {
  ...
  try
  {
    ...
    return res.status(200).send({ "message": "Juriile au fost adaugate la echipa." })
  }
  catch(err)
  {
    next(err);
...
router.delete('/:id/juries', async (req, res, next) => {
  ...
  try
  {
    ...
    return res.status(200).send({ "message": "Juriile echipei au fost schimbate cu succes." })
  }
  catch(err)
  {
    next(err);
...
router.post('/:id/grade', async (req, res, next) =>
{
  ...
  try
  {
    ...
    return res.status(200).json({ message: "A fost schimbata nota echipei." });
  }
  catch(err)
  {
    next(err);
