...
export default function TeamsTable({ useAuthHandler, onlyStudents, onlyProfessors }) {
    ...
    const deleteTeam = async id => {
        ...
        }, resp => resp.status === 200 ? setSuccess(`Ati scos echipa cu succes.`) : setError('Nu s-a putut scoate echipa. Va rugam sa incercati mai tarziu.'));
    };

    const renderRemoveButton = id => {
        if (currentUser && currentUser.is_professor) {
            return (<th><Button size="xs" color="danger" onClick={async () => await deleteTeam(id)}>Inlaturati</Button></th>);
        }
    ...
    const renderAddTeamButton = _ => {
      ...
      const addTeamModal = _ => (
        <Modal isOpen={isAddTeamModalOpen} toggle={_ => toggleAddTeamModal(false)}>
          <ModalHeader>Adaugati echipa</ModalHeader>
          <ModalBody>
            <FormGroup>
            ...
            <FormGroup>
              <Label for="project-name">Nume proiect</Label>
              <Input id="project-name" name="project-name" type="text" placeholder="Proiect 1" />
            </FormGroup>
          </ModalBody>
          <ModalFooter>
            <Button onClick={_ => toggleAddTeamModal(false)}>Anulare</Button>
            <Button color="primary" onClick={handleSave}>Salvare</Button>
          </ModalFooter>
        </Modal>
      );
      ...
      return (
        <div className="py-2 pb-3">
          {addTeamModal()}
          <Button color="success" onClick={_ => toggleAddTeamModal(true)}>Adaugati echipa</Button>
        </div>
      )
    };
