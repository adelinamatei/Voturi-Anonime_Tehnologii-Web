...
export default function Team({ useAuthHandler }) {
    ...
    const renderDeliverables = () => {
        return (<Card>
            <CardHeader>
                <CardTitle tag="h3">Livrabile</CardTitle>
            </CardHeader>
            <CardBody>
                <Table responsive striped hover>
                    <thead>
        ...
        const delivModal = () => {
            return (
                <Modal isOpen={isDelivModalOpen} toggle={_ => toggleDelivModal(false)}>
                    <ModalHeader>Adaugati livrabile</ModalHeader>
                    <ModalBody>
                    ...
                    <ModalFooter>
                        <Button onClick={() => toggleDelivModal(false)}>Anulare</Button>
                        <Button color="primary" onClick={submit}>Salvare</Button>
                    </ModalFooter>
                </Modal>
            )
        }
        ...
        const usersModal = () => {
            return (
                <Modal isOpen={isMembersModalOpen} toggle={_ => toggleMembersModal(false)}>
                    <ModalHeader>Adaugati membru</ModalHeader>
                    <ModalBody>
                        <FormGroup>
                            ...
                            <tbody>
                                {users && users.map((d, i) => (
                                    <tr key={i}>
                                        <th>{d.surname}</th>
                                        <th>{d.name}</th>
                                        <th><Button color="success" onClick={async () => await addMember(d.username, d.id)}>Adaugati</Button></th>
                                    </tr>
                                ))}
                            </tbody>
                        </Table>
