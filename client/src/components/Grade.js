    ...
    <Table responsive striped hover>
      ...
      <tbody>
          ...
          <tr key={t.id}>
            ...
            <th>
              <Button color="primary"
                ...
                {getGradeFromJury(teams.userId, t.Jury) || 'Nu exista nota'}
              </Button>
            </th>
        ...
        {(!teams || teams.values.filter(t => t.Jury.grades.some(g => g.userId === teams.userId)).length < 1) &&
          <tr>
            <th colSpan="5">Nu au fost gasite echipe atribuite</th>
          </tr>}
      </tbody>
    </Table>
  );
...
const createEditModal = (authHandler, data, setData, grade, setGrade, setSuccess, setError) =>
{
  ...
  return (
    <Modal isOpen={data.isOpen} toggle={_ => setData({ isOpen: false })}>
      <ModalHeader>Editeaza nota</ModalHeader>
      <ModalBody>
        <Table responsive striped hover>
          <thead>
            <tr>
          ...
          <tbody>
            ...
            {!data.deliverables && (
              <tr>
                <th colSpan="3">Nu au fost gasite livrabile.</th>
              </tr>
            )}
          </tbody>
        </Table>
      ...
      <ModalFooter>
        <Button onClick={_ => setData({ isOpen: false })}>Anulare</Button>
        <Button color="primary" onClick={async _ => await saveGrade()}>Salvare</Button>
      </ModalFooter>
    </Modal>
  );
}
...
export default function Grade({ useAuthHandler })
{
  ...
  return (
    <Container className="py-2">
      ...
      <Card>
        <CardHeader>Proiecte date pentru notare</CardHeader>
        <CardBody>
